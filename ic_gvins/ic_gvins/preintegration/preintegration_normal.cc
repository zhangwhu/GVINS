...
Eigen::MatrixXd PreintegrationNormal::residualJacobianPose0(const IntegrationState &state0,
    ...
    jaco.block(0, 0, 3, 3) = -state0.q.inverse().toRotationMatrix();                                        // 位置---位置0
    jaco.block(0, 3, 3, 3) =
        Rotation::skewSymmetric(state0.q.inverse() * (state1.p - state0.p - state0.v * delta_time_ -
                                                      0.5 * gravity_ * delta_time_ * delta_time_));         // 位置---姿态0
    jaco.block(3, 3, 3, 3) =
        Rotation::skewSymmetric(state0.q.inverse() * (state1.v - state0.v - gravity_ * delta_time_));       // 速度---姿态0
    jaco.block(6, 3, 3, 3) =
        -(Rotation::quaternionleft(state1.q.inverse() * state0.q) * Rotation::quaternionright(corrected_q_))// 姿态---姿态0
...
Eigen::MatrixXd PreintegrationNormal::residualJacobianPose1(const IntegrationState &state0,
    ...
    jaco.block(0, 0, 3, 3) = state0.q.inverse().toRotationMatrix();                                         // 位置---位置1
    jaco.block(6, 3, 3, 3) =
...
Eigen::MatrixXd PreintegrationNormal::residualJacobianMix0(const IntegrationState &state0,
    ...
    Eigen::Matrix3d dp_dbg = jacobian_.block<3, 3>(0, 9);               // PVA对零偏的偏导 
    ...
    jaco.block(0, 0, 3, 3) = -state0.q.inverse().toRotationMatrix() * delta_time_;                          // 位置---速度0
    jaco.block(0, 3, 3, 3) = -dp_dbg;                                                                       // 位置---陀螺零偏0
    jaco.block(0, 6, 3, 3) = -dp_dba;                                                                       // 位置---加计零偏0
    jaco.block(3, 0, 3, 3) = -state0.q.inverse().toRotationMatrix();                                        // 速度---速度0
    jaco.block(3, 3, 3, 3) = -dv_dbg;                                                                       // 速度---陀螺零偏0
    jaco.block(3, 6, 3, 3) = -dv_dba;                                                                       // 速度---加计零偏0
    jaco.block(6, 3, 3, 3) =
        -Rotation::quaternionleft(state1.q.inverse() * state0.q * delta_state_.q).bottomRightCorner<3, 3>() * dq_dbg; // 姿态---陀螺零偏0
    jaco.block(9, 3, 3, 3)  = -Eigen::Matrix3d::Identity();                                                 // 陀螺---陀螺零偏0
    jaco.block(12, 6, 3, 3) = -Eigen::Matrix3d::Identity();                                                 // 加计---加计零偏0

    jaco = sqrt_information_ * jaco;
    return jaco;
}
// Mix: 速度+零偏
// 0-2: 位置    3-5: 速度   6-8: 姿态   9-11: 零偏  12-14: 零偏
Eigen::MatrixXd PreintegrationNormal::residualJacobianMix1(const IntegrationState &state0,
                                                           const IntegrationState &state1, double *jacobian) {
    Eigen::Map<Eigen::Matrix<double, NUM_STATE, NUM_MIX, Eigen::RowMajor>> jaco(jacobian);
    jaco.setZero();

    jaco.block(3, 0, 3, 3)  = state0.q.inverse().toRotationMatrix();                                        // 速度---速度1  
    jaco.block(9, 3, 3, 3)  = Eigen::Matrix3d::Identity();                                                  // 陀螺---陀螺零偏1
    jaco.block(12, 6, 3, 3) = Eigen::Matrix3d::Identity();                                                  // 加计---加计零偏1
...
void PreintegrationNormal::integrationProcess(unsigned long index) {
    ...
    // 连续状态积分和预积分
    integration(imu_pre, imu_cur);          // 这里好像没有什么差异性
...
void PreintegrationNormal::setNoiseMatrix() {
    ...
    noise_.block<3, 3>(0, 0) = Vector3d(parameters_->gyr_arw.array().pow(2)).asDiagonal(); // nw
    noise_.block<3, 3>(3, 3) = Vector3d(parameters_->acc_vrw.array().pow(2)).asDiagonal(); // na
    noise_.block<3, 3>(6, 6) = 
        Vector3d(2 * parameters_->gyr_bias_std.array().pow(2) / parameters_->corr_time).asDiagonal(); // nbg   Rx=q/(2*B)
    noise_.block<3, 3>(9, 9) =
        Vector3d(2 * parameters_->acc_bias_std.array().pow(2) / parameters_->corr_time).asDiagonal(); // nba
}
