...
void Camera::undistortPoints(std::vector<cv::Point2f> &pts) {
    cv::undistortPoints(pts, pts, intrinsic_, distortion_, Mat(), intrinsic_);          // 这个玩意的含义是什么，不是特别了解
}

void Camera::distortPoints(std::vector<cv::Point2f> &pts) const {                       // 正常的点 >> 增加畸变
    for (auto &pt : pts) {
...
Vector3d Camera::pixel2cam(const cv::Point2f &pixel) const {                // 归一化坐标
