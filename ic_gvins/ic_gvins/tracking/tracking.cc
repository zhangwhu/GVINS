...
Tracking::Tracking(Camera::Ptr camera, Map::Ptr map, Drawer::Ptr drawer, const string &configfile,
    ...
    track_min_features_    = config["track_min_features"].as<int>();
    ...
    block_indexs_.emplace_back(std::make_pair(col, row));           // 分块索引问题
...
bool Tracking::preprocessing(Frame::Ptr frame) {
    ...
    frame_pre_ = frame_cur_;                        // 预处理：将上一帧进行移除
...
TrackState Tracking::track(Frame::Ptr frame) {      
    ...
    // 预处理
    if (!preprocessing(std::move(frame))) {         // 转移对象的所有权（无内存的搬迁和拷贝）    
        return track_state;
    }
    ...
    if (isinitializing_) {                          
        ...
        if (frame_cur_->stamp() - (int)frame_cur_->stamp() < 0.02) {     // 避免和GNSS发生节点冲突
            return TRACK_INITIALIZING;
        }
        if (static_cast<int>(pts2d_ref_.size()) < 100) {
            LOGW << "Reset initialization " << frame_cur_->stamp();
            frame_ref_ = frame_cur_;  
            featuresDetection(frame_ref_, false);
            frame_ref_->setKeyFrame(KEYFRAME_NORMAL);   
            showTracking(); 
        ...
        // 检查关键帧类型
        auto keyframe_state = checkKeyFrameSate();  // 保持和参考帧的关系即可        

        // 正常关键帧, 需要三角化路标点
        if ((keyframe_state == KEYFRAME_NORMAL) || (keyframe_state == KEYFRAME_REMOVE_OLDEST)) {
            // 三角化补充路标点
            triangulation();                        // 三角化：1.转移中途加入的特征点 2.特征点三角化 （没有进行初始化）              
        } else {
            // 添加新的特征 
            featuresDetection(frame_cur_, true);    // 1.无较大位移 2.时间间隔较短（特征点的参考帧是如何传递过去的）         
        }

        // 跟踪失败, 路标点数据严重不足
        if (doResetTracking()) {                    // 跟踪到的地图点太少                                
            makeNewFrame(KEYFRAME_NORMAL);                   
            return TRACK_LOST;                      
        }
        ...
        if (keyframe_state != KEYFRAME_NONE) {              
            makeNewFrame(keyframe_state);           // 加入一个新关键帧       
        }
...
void Tracking::makeNewFrame(int state) {
    frame_cur_->setKeyFrame(state);                  // 帧
    isnewkeyframe_ = true;

    // 仅当正常关键帧才更新参考帧
    if ((state == KEYFRAME_NORMAL) || (state == KEYFRAME_REMOVE_OLDEST)) { 
        frame_ref_ = frame_cur_;                     // 参考帧没有发生变化

        featuresDetection(frame_ref_, true);         // 加入新特征
    }
}
...
keyFrameState Tracking::checkKeyFrameSate() {
    ...
    if (dt < TRACK_MIN_INTERVAl) {              // 相邻时间太短不进行关键帧处理
    ...
    if (parallax > track_min_parallax_) {       // 视差问题（主要查看是否进行了空间移动）
        // 新的关键帧, 满足最小像素视差

        keyframe_state = map_->isWindowFull() ? KEYFRAME_REMOVE_OLDEST : KEYFRAME_NORMAL;           //如果地图满了，就删除最后的关键帧

        LOGI << "Keyframe at " << Logging::doubleData(frame_cur_->stamp()) << ", mappoints "
             << frame_cur_->numFeatures() << ", interval " << dt << ", parallax " << parallax;
    } else if (dt > track_max_interval_) {              // 视差太小 （但是可能在移动）
        // 普通观测帧, 非关键帧
    ...
    // 切换上一关键帧, 用于时间间隔计算
    if (keyframe_state != KEYFRAME_NONE) {
        last_keyframe_ = frame_cur_;                    //次新关键帧

        // 更新路标点在观测帧中的使用次数
        for (auto &mappoint : tracked_mappoint_) {      //关键帧的状态进行更新
...
// 1.当前帧和以往帧联系消除 2.当前帧特征点消除 3.当前帧转为参考帧 4.初始化标志
bool Tracking::doResetTracking() {
    if (static_cast<int>(frame_cur_->numFeatures()) < track_min_features_) {         
        for (auto &mappoint : frame_cur_->unupdatedMappoints()) {
            for (auto &observation : mappoint->observations()) {
                auto feat = observation.lock();         
                if (!feat || feat->isOutlier()) {
                    continue;
                }
                feat->getFrame()->delFeature(mappoint->id());        
            }
        }
...
bool Tracking::trackMappoint() {                        
    ...
    auto features = frame_pre_->features();             // 上帧匹配的地图点
    for (auto &feature : features) {
        auto mappoint = feature.second->getMapPoint();  // 上帧带有路标点的特征
        if (mappoint && !mappoint->isOutlier()) {       // 地图点不可用
    ...
    reduceVector(pts2d_map_undis, status);                          // 地图点、无畸变
    ...
    for (size_t k = 0; k < pts2d_matched_undis.size(); k++) {      // 匹配后的点，重新进行矫正
        ...
        auto feature  = Feature::createFeature(frame_cur_, {velocity.x(), velocity.y()}, pts2d_matched_undis[k],
                                               pts2d_matched[k], FEATURE_MATCHED);      // 畸变和去畸变问题    
        mappoint->addObservation(feature);
        feature->addMapPoint(mappoint);
        frame_cur_->addFeature(mappoint->id(), feature);            // 关键帧同时包含特征和地图点            
...
bool Tracking::trackReferenceFrame() {
    ...
    // 补偿旋转预测
    Matrix3d r_cur_pre = frame_cur_->pose().R.transpose() * frame_pre_->pose().R;    // 补偿位移（修改）！
    ...
    pts2d_cur_.clear();
    for (const auto &pp_pre : pts2d_new_undis) {        // pts2d_cur：表示当前帧跟踪到的一些（未地图点化）的特征点
    ...
    LOGI << "Track " << pts2d_new_.size() << " reference points" << " num_feature" << frame_cur_->numFeatures();
...
void Tracking::featuresDetection(Frame::Ptr &frame, bool ismask) {                      
    ...
    // 特征点足够则无需提取
    int num_features = (ismask) ? static_cast<int>(frame->features().size()+pts2d_ref_.size()) : 0; 
    ...
    if (ismask) {
        // 已经跟踪上的点 
        for (const auto &pt : frame->features()) {          // By zh 2023-06-13
...
bool Tracking::triangulation() {
    ...
    for (size_t k = 0; k < pts2d_cur_.size(); k++) {
        ...
        if (frame_ref->id() > frame_ref_->id()) {                       // 中途加入的特征 >>> 将其转移到当前帧上
            // 中途添加的特征, 修改参考帧
        ...
        if (map_->isWindowNormal() && !map_->isKeyFrameInMap(frame_ref)) {      // 参考帧被删掉
            // LOGW << "Not in map " << frame_ref->stamp() << "  "<< map_->oldestKeyFrame()->stamp() << "  "
            //      << map_->latestKeyFrame()->stamp();
            status.push_back(0);
            num_outtime++;                  
            continue;
        }
        ...
        mappoint->addObservation(feature);                  // 两个都是弱引用
        feature->addMapPoint(mappoint);
        frame_ref->addFeature(mappoint->id(), feature);     // 强引用    
        mappoint->increaseUsedTimes();

        // 新三角化的路标点缓存到最新的关键帧, 不直接加入地图
        frame_cur_->addNewUnupdatedMappoint(mappoint);      // 地图点强引用
    }

    // 由于视差不够未及时三角化的角点
    reduceVector(pts2d_ref_, status);                       // 三角化后的点不会加入待处理列表内      
    reduceVector(pts2d_ref_frame_, status);
    reduceVector(pts2d_cur_, status);
    reduceVector(velocity_ref_, status);
...
double Tracking::keyPointParallax(const cv::Point2f &pp0, const cv::Point2f &pp1, const Pose &pose0,
    ...
    // 补偿掉旋转
    Vector3d pc01 = pose1.R.transpose() * pose0.R * pc0;            // 首先转为归一化坐标                          

    // 像素大小
    return (pc01.head<2>() - pc1.head<2>()).norm() * camera_->focalLength();     // 视差转到像素坐标系的差别  
}
    ...
    for (auto &feature : features) {
        ...
        if (mappoint && !mappoint->isOutlier()) {
            ...
            if (feat && !feat->isOutlier()) {
                ...
                if (frame && (frame == frame_cur_)) {           // 获得与参考帧间的视差，这个有什么意义不是很清楚
                    // 对应同一路标点在当前帧的像素观测
