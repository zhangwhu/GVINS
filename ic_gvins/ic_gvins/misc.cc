...
size_t MISC::getImuWindowIndex(const std::vector<IMU> &window, double time) {
    // 返回时间大于输入的第一个索引

    if (window.empty() || (window.front().time > time) || (window.back().time <= time)) {
        return 0;
    }
    ...
    while (true) {
        ...
        auto first  = window[mid - 1].time;
        auto second = window[mid].time;
...
void MISC::writeNavResult(const IntegrationConfiguration &config, const IntegrationState &state,
                          const Vector3d &antlever, const FileSaver::Ptr &navfile, 
                          const FileSaver::Ptr &errfile, const FileSaver::Ptr &trajfile) {
    // static int counts = 0;
    // if (counts++ % 10) {
    //     return;
    // }
    if (fabs(state.time - (int)state.time) > 0.001) return;

    // 保存结果
    vector<double> result;
    auto state0  = state;
    double time  = state0.time;
    state0.p     = state0.p + state0.q.toRotationMatrix() * antlever;
    Pose global  = Earth::local2global(config.origin, Pose{state0.q.toRotationMatrix(), state0.p});
    
    Vector3d pos = global.t;
    Vector3d att = Rotation::matrix2euler(global.R);
    Vector3d vel = state0.v;
    Vector3d bg  = state0.bg;           
    Vector3d ba  = state0.ba; 
...

    {
        ...
        if (config.iswithscale) {
            Vector3d sg = state0.sg * 1e6;
            Vector3d sa = state0.sa * 1e6;
...

    {
        ...
        result.push_back(time);
        result.push_back(state0.p[0]);
        result.push_back(state0.p[1]);
        result.push_back(state0.p[2]);
        result.push_back(state0.q.x());
        result.push_back(state0.q.y());
        result.push_back(state0.q.z());
        result.push_back(state0.q.w());
