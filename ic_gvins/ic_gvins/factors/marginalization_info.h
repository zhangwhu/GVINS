...
public:
    ...
    void addResidualBlockInfo(const std::shared_ptr<ResidualBlockInfo> &blockinfo) {    
        factors_.push_back(blockinfo);                                              // 这里可以直接进行数据处理

        const auto &parameter_blocks = blockinfo->parameterBlocks();                // 块 + 块大小
        const auto &block_sizes      = blockinfo->parameterBlockSizes();            

        for (size_t k = 0; k < parameter_blocks.size(); k++) {                      // 映射关系
        ...
        for (int index : blockinfo->marginalizationParametersIndex()) {             
            parameter_block_index_[parameters_ids_[reinterpret_cast<long>(parameter_blocks[index])]] = 0;           // 左值|右值概念
        }
    }
    ...
    std::vector<double *> getParamterBlocks(std::unordered_map<long, double *> &address) {
        ...
        for (const auto &block : parameter_block_index_) {                             
            ...
            if (block.second >= marginalized_size_) {            
                remained_block_data_.push_back(parameter_block_data_[block.first]);     // 状态（副本）
                remained_block_size_.push_back(parameter_block_size_[block.first]);     // 块大小
                remained_block_index_.push_back(parameter_block_index_[block.first]);   // 索引
                remained_block_addr.push_back(address[block.first]);                    // 状态（正本）将其对齐               
            }
        }
...
private:
    ...
    void linearization() {
        ...
        // J0 = S^{1/2} * V^T
        linearized_jacobians_ = S_sqrt.asDiagonal() * saes2.eigenvectors().transpose();   // 参考VINS的margin+factor.pdf
    ...
    void constructEquation() {
        ...
        for (const auto &factor : factors_) {
            for (size_t i = 0; i < factor->parameterBlocks().size(); i++) {
                ...
                Eigen::MatrixXd jacobian_i = factor->jacobians()[i].leftCols(rows);       // 前rows列      
        ...
        for (const auto &factor : factors_) {                   
            ...
            for (size_t k = 0; k < block_sizes.size(); k++) {   
                ...
                if (parameter_block_data_.find(id) == parameter_block_data_.end()) {
                    ...
                    parameter_block_data_[id] = data;           // 将数据副本保存下来
                }
            }
        }
    }
