...
public:
    ...
    explicit PreintegrationFactor(std::shared_ptr<PreintegrationBase> preintegration)
        ...
        // parameter
        *mutable_parameter_block_sizes() = preintegration_->numBlocksParameters();      // 每个参数块大小
    ...
    bool Evaluate(const double *const *parameters, double *residuals, double **jacobians) const override {
        ...
        // residual
        preintegration_->evaluate(state0, state1, residuals);       // 残差+雅克比矩阵
...
private:
    std::shared_ptr<PreintegrationBase> preintegration_;                // 标准格式
};
