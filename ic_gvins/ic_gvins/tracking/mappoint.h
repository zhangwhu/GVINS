...
enum MapPointType {
    ...
    MAPPOINT_DEPTH_INITIALIZED = 2, //深度初始化
    MAPPOINT_FIXED             = 3,
};
...
public:
    ...
    int observedTimes() const {
        return observed_times_;                     // 一共跟踪了多少次
    }
...
private:
    ...
    int optimized_times_;           // 进入优化次数
    int used_times_;                // 可用残差边个数
    int observed_times_;            // 被关键帧和非关键帧观测到的次数
    bool isoutlier_;
