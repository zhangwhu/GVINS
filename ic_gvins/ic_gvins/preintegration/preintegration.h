...
public:
    ...
    //     return {};
    // }
    static std::shared_ptr<IntegrationStateData> 
        stateToData(const IntegrationState &state, PreintegrationOptions options) {
        std::shared_ptr<IntegrationStateData> statedata;
        if (options == PREINTEGRATION_NORMAL) {
            statedata = std::make_shared<IntegrationStateData>(PreintegrationNormal::stateToData(state));
        } else if (options == PREINTEGRATION_ODO) {
            statedata = std::make_shared<IntegrationStateData>(PreintegrationOdo::stateToData(state));
        } else if (options == PREINTEGRATION_EARTH) {
            statedata = std::make_shared<IntegrationStateData>(PreintegrationEarth::stateToData(state));
        } else if (options == PREINTEGRATION_EARTH_ODO) {
            statedata = std::make_shared<IntegrationStateData>(PreintegrationEarthOdo::stateToData(state));
        }
        return statedata;
    }
